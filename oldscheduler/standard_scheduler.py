...
def main():
    usage = "usage: %prog [options] behaviour_dir"
    parser = ArgumentParser(usage)

    parser.add_argument("dirname", metavar="DIRNAME")
    parser.add_argument("-w", "--write-test", dest="write",
                        help="write results in CSV", metavar='WRITE')
    ...
    print result
    if options.write:
        with open('/home/cossmic/Tesi/risultati/results_nosmart.csv', 'ab') as csvfile:
            b = best_ind
            best_ind[:] = [int(x) for x in best_ind]
            reswriter = csv.writer(csvfile, delimiter=',',
                                    quotechar='"', quoting=csv.QUOTE_MINIMAL)
            reswriter.writerow([result['pvtotal'] ,result['energy_out'] ,result['energy_in'] ,result['self_consumption'] ,
                                result['max_consumption'], mean, qn, MIN, MAX, b, end-start])
        exit(0)
    myProblem.plotpower()
    plt.show()
