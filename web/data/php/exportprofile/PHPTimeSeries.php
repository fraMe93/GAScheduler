...
class PHPTimeSeries
{
    ...
    */
    ...
    private $dir = "/var/www/virtualDevices/Interface/data/php/feed/";
    ...
    public function create($feedid,$options)
    {
        ...
        if (!$fh) {
            error_log("PHPTimeSeries:create could not create data file feedid=$feedid");
        }
        fclose($fh);
    ...
    {
        error_log("PHPTimeSeries:post feedid=$feedid time=$time value=$value");
        
        // Get last value
        $fh = fopen($this->dir."feed_$feedid.MYD", 'rb');
        if (!$fh) {
            error_log("PHPTimeSeries:post could not open data file feedid=$feedid");
            return false;
        }
        ...
        if ($csize!=$filesize) {
            ...
            error_log("PHPTimeSeries:post filesize not integer multiple of 9 bytes, correcting feedid=$feedid");
    ...
    private function fopendata($filename,$mode)
    {
        ...
        if (!$fh) {
            error_log("PHPTimeSeries:fopendata could not open $filename");
            return false;
        }
        
        if (!flock($fh, LOCK_EX)) {
           error_log("PHPTimeSeries:fopendata $filename locked by another process");
            fclose($fh);
            return false;
        }
...
//########      
   public function exportcsv($feedid,$outfile)
    {
        $feedid = (int) $feedid;
        ...
        $exportfh = fopen($this->dir.$outfile, 'w' );
       ...
       while (!feof($fh))
        {
            ...
            fwrite($exportfh, $array['time'].",".number_format($array['value'],2)."\n");
